...
- **Active provider:** `AI_PROVIDER` / `NEXT_PUBLIC_AI_PROVIDER` (`foundry` ≡ `azure`). Microsoft Foundry is the default when Azure credentials exist in env. User selection comes from `yula_ai_config` (localStorage) + the provider list in the model popover (`listConfiguredProviders`). With no stored selection, no `provider` is written to the request body; the server trusts env.
