...
class DuckDbClient {
  ...
  private getWorker(): Worker {
    if (!this.worker) {
      this.worker = new Worker(
        new URL("./wasm-sql.worker.ts", import.meta.url),
        { type: "module" }
      )
  ...
  async exportReportTable(options: {
    ...
    link.href = url
    const defaultExt = res.format === "gz" ? ".csv.gz" : `.${res.format || "csv"}`
    link.download = res.fileName || `${fileName}${defaultExt}`
...
export const wasmSqlClient = new DuckDbClient()
export { DuckDbClient as WasmSqlClient }
