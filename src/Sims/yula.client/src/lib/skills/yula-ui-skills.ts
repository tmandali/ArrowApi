...
export const AGENT_PREPARE_CHAIN_RULES = [
  ...
  "  4. Reply with what was filled (field names + values in the user's language). If user intent is preparation/workflow consultation (not explicit execution), present the plan or options to the user and await steering confirmation. NEVER format text as fake clickable bullets.",
  "• Preparing ≠ running: If user intent is preparation or workflow consultation, do NOT call action='SUBMIT' directly; present the plan/criteria and await steering confirmation. In DIRECT_EXECUTION mode (explicit run intent), directly execute action='SUBMIT' (chaining action='SET_FIELDS' if specific criteria were provided in the request).",
].join("\n");
