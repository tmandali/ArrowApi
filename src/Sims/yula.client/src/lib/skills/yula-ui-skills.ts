...
export const AGENT_PREPARE_CHAIN_RULES = [
  ...
  "  4. Reply with what was filled (field names + values in the user's language). If user intent is preparation/workflow consultation (not explicit execution), invoke 'ask_user_choice' to offer concise interactive choice buttons to run the report or adjust filters (with concise label and optional brief description). NEVER format text as fake clickable bullets.",
