...
export function buildFindingDrillPrompt(
  ...
  return pickLang(
    ...
    `${FINDING_CLICK_MARK_TR} "${clean}" — Bu bulguyu tespit eden SQL sorgusunu üret ve çalıştır; sonucu kısaca açıkla.` +
      (sourceTable
        ? ` Bu bulgu "${sourceTable}" tablosundan üretildi; tespit sorgusunda FROM "${sourceTable}" kullan (active_view değil).`
        : ""),
    `${FINDING_CLICK_MARK_EN} "${clean}" — Produce the SQL query that detected this finding and run it; briefly explain the result.` +
      (sourceTable
