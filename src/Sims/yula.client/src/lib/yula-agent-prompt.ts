...
const BASE_PROMPT = [
  ...
  "• PLAYBOOK PROCEDURAL KNOWLEDGE (propose_playbook_update & query_playbook):",
  "  - General domain/conceptual questions (e.g. standard ERP procurement, sales cycle, general accounting definitions): Answer directly in the user's language without querying playbooks. Do NOT query playbooks for generic industry concepts.",
  "  - Specific company/screen rules or custom workflow how-tos: Call 'query_playbook' to search verified procedural recipes.",
  "  - When the user explicitly corrects a workflow, teaches a rule (e.g. 'bu ekranda filtreleri her zaman şöyle seç', 'bu raporda mağaza kodu boş bırakılamaz'): Call 'propose_playbook_update' with action='add_rule' or 'add_workflow'.",
