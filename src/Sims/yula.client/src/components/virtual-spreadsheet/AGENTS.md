...
Bu doküman, `VirtualSpreadsheet` ve alt modüllerinde (`src/components/virtual-spreadsheet/`) geliştirme, hata ayıklama veya refactoring yapacak geliştiriciler ve AI ajanları için mimari standartları ve korunması gereken temel değişmezleri (invariants) tanımlar.
...
```
src/components/virtual-spreadsheet/
...
pnpm run test:grid
```
Test suite (`src/components/virtual-spreadsheet/tests/`) şunları garanti eder:
