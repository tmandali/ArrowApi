...
/** Sonuç evresi çipleri — grid AÇIKKEN: analize odaklı, rapor başlatma YOK. */
...
export function YulaQuickActionChips() {
  ...
  // Evre tespiti: grid AÇIK → sonuç evresi (analiz çipleri); YOK → workspace
  // evresi (rapor başlatma çipleri). Sonuç ekranında "Raporu hazırla" çipi
  // anlamsızdır — veri zaten ekranda.
  const hasGrid = !!spec && spec.columns.length > 0

  const chips = React.useMemo<QuickChip[]>(() => {
    if (hasGrid) return RESULT_CHIPS
    return [
