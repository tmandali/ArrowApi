...
import { wasmStreamManager } from "../services/wasm-stream-manager"
import type { ColumnSortConfigs } from "@/components/virtual-spreadsheet"
import { type WasmSqlColumnMeta, isCustomQueryActive } from "./wasm-sql-types"

export type UseWasmStreamSubscriptionOptions<T = Record<string, unknown>> = {
...
export type UseDuckStreamSubscriptionOptions<T = Record<string, unknown>> =
  UseWasmStreamSubscriptionOptions<T>

export function useWasmStreamSubscription<T = Record<string, unknown>>({
  jobId,
  jobUrl,
  tableName,
  expectedTotalRows,
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = wasmStreamManager.subscribe(
      {
        jobId,
        jobUrl,
        tableName,
      ...
      (state) => {
        ...
        if ((state.isTableReady && state.streamedRows > 0) || state.isComplete || state.isFromCache) {
          ...
          if (shouldQuery) {
            markTableReady(true)
            void wasmSqlClient.describeTable(tableName).then((discovered) => {
