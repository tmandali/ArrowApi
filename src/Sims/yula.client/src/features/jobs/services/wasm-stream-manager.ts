...
export class WasmStreamManager {
  ...
  async cancel(jobId: string): Promise<void> {
    ...
    if (session) {
      ...
      await wasmSqlClient.dropTable(session.tableName).catch(() => {})
  ...
  async restart(options: {
    ...
    await wasmSqlClient.resetDatabase().catch(() => {})
  ...
  private scheduleCleanup(session: StreamSessionInternal): void {
    ...
    session.cleanupTimer = setTimeout(() => {
      ...
      if (session.listeners.size > 0) return
      session.abortController.abort()
      void wasmSqlClient.dropTable(session.tableName).catch(() => {})
  ...
  private async startBackgroundStream(
    ...
    try {
      if (options?.resetDatabase) {
        await wasmSqlClient.resetDatabase().catch((err) => {
          console.warn("[WasmStreamManager] resetDatabase uyarısı:", err)
        })
      }

      if (!forceServerFetch) {
        // 1. Tablo veya View DuckDB'de zaten mevcut mu kontrol et (0ms)
        const check = await wasmSqlClient.checkTableExists(tableName)
        ...
        if (hasParquet) {
          try {
            ...
            if (partFiles.length > 0) {
              const res = await wasmSqlClient.registerParquetPartsView({
                tableName,
                jobId,
                partFiles,
              })
      ...
      const result = await arrowStreamClient.startStream({
        ...
        onProgress: async (prog) => {
          ...
          if (!session.isTableReady && prog.partFiles && prog.partFiles.length > 0) {
            try {
              const viewRes = await wasmSqlClient.registerParquetPartsView({
                tableName,
                jobId,
      ...
      if (!isAborted) {
        ...
        console.error("WasmStreamManager stream error:", err)
...
export const wasmStreamManager = new WasmStreamManager()
