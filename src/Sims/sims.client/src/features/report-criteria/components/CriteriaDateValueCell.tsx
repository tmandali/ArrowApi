...
function parseRangeValue(
  ...
  if (separator && trimmed.includes(separator)) {
    const { from: fromText, to: toText } = splitRangeCellValue(
      trimmed,
      separator
    )
    const from = parseDateToken(fromText)
    const to = parseDateToken(toText)
...
export function CriteriaDateValueCell({
  ...
  const selectedRange = React.useMemo(
    () => parseRangeValue(value, rangeSplit),
    [value, rangeSplit]
  )
  ...
  const commitRange = (range: DateRange | undefined) => {
    ...
    const fromToken = formatDateToken(range.from, useCompact)
    if (!range.to || range.to.getTime() === range.from.getTime()) {
      onChange(fromToken)
      return
    }
    const sep = rangeSplit || ".."
    onChange(`${fromToken}${sep}${formatDateToken(range.to, useCompact)}`)
  }
