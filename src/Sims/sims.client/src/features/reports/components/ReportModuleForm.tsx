...
import { workspaceKeyFromPath } from "@/lib/workspace"
import {
  isTerminalJobStatus,
...
import genericReportCriteriaSchema from "../schemas/generic-report-criteria.schema.json"
import type { ReportModuleJobSession } from "./ReportModuleFilter"
...
export function ReportModuleForm({
  ...
  const navigateBase = `/${workspace}/${slug}`
  const jobsEndpoint = `/api/arrow/jobs/${slug}`
  const { trackJob, waitUntilTerminal } = useJobSync()

  const schema = React.useMemo(() => {
    const base = genericReportCriteriaSchema as JsonSchemaObject
    return {
      ...base,
      title: `${title} Criteria`,
      description: `Define filter criteria for ${title}.`,
      "x-job-endpoint": jobsEndpoint,
  ...
  const trackReportJob = React.useCallback(
    (
      job: {
        id: string
        name?: string
      ...
      trackJob({
        ...
        name: job.name || slug,
        title,
        href: `${navigateBase}/${job.id}`,
        status: job.status || "Queued",
        eventsUrl: job.eventsUrl,
        jobUrl: job.jobUrl,
        createdAt: job.createdAt || new Date().toISOString(),
        notificationType: "report",
        workspace: workspaceKeyFromPath(`/${workspace}`),
    ...
    [navigateBase, slug, title, trackJob, workspace]
  )
  ...
  const followJob = React.useCallback(
    ...
    [trackReportJob, waitUntilTerminal, publishFocused, patchLive]
  )
  ...
  React.useEffect(() => {
    allowEntryResumeRef.current = true
    preferCriteriaRef.current = true
    setComposing(true)
    publishFocused(null)
    setPendingJobs([])
    setListRefreshToken((n) => n + 1)
  }, [slug, workspace, publishFocused])
  ...
  React.useEffect(() => {
    ...
    const resume = async () => {
      try {
        const tracked = selectPendingJobByName(
          useActiveJobsStore.getState().jobs,
          slug,
          workspaceKeyFromPath(`/${workspace}`)
        )
        ...
        if (!job) {
          try {
            const page = await listArrowJobs(jobsEndpoint, { take: 50 })
            if (
    ...
    void resume()
  }, [followJob, jobsEndpoint, slug, workspace])
  ...
  const jobSession: ReportModuleJobSession = {
    activeJobId,
    activeLiveStatus,
    activeRequestJson,
    activeRunEvents,
  ...
  return (
    <ItemForm
      ...
      onReportJobCreated={handleJobCreated}
      reportJobSession={jobSession}
    />
  )
}
