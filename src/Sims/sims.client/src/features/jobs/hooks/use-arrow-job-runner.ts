...
export function useArrowJobRunner(options: ArrowJobRunnerOptions) {
  ...
  const handleSelectJob = React.useCallback(
    (job: ArrowJobStatus | null) => {
      ...
      if (liveByJobRef.current.has(job.id)) return
      ...
      void fetchJobRequest(job.id).then((req) => {
        if (!req) return
        patchLive(job.id, { requestJson: prettyJson(req) })
      })

      if (isInFlightStatus(job.status) && !controllersRef.current.has(job.id)) {
        void fetchJobRequest(job.id).then((req) => {
          void followJob(job, req ?? {})
        })
      }
    },
