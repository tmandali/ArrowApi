...
function jobHref(jobId: string): string {
  return `${STOCK_BALANCE_PATH}/${jobId}`
}
...
export function StockBalanceForm() {
  ...
  /** True on entry / New / empty list; false after Run or picking an execution. */
  ...
  /** Cancel in-flight entry resume when user clicks New. */
  ...
  const trackBalanceJob = React.useCallback(
    (
      job: {
        id: string
        name?: string
      ...
      trackJob({
        ...
        workspace: "/stock",
        successTitle: "Stock Balance Ready",
        successDescription:
          "Stock Balance job tamamlandı. Açmak için bildirime tıklayın.",
        failureTitle: "Stock Balance Failed",
        payload,
      })
    },
    [trackJob]
  ...
  const followJob = React.useCallback(
    async (job: ArrowJobStatus, request: Record<string, unknown>) => {
      // Do not abort other in-flight jobs — queue multiple runs.
    ...
    [trackBalanceJob, waitUntilTerminal, publishFocused, patchLive]
  )
  ...
  // Page entry: if a Stock Balance job is already Running/Queued, open it.
  React.useEffect(() => {
    ...
    const resume = async () => {
      try {
        const tracked = selectPendingStockBalanceJob(
        ...
        if (!job) {
          const page = await listArrowJobs(STOCK_BALANCE_JOBS, { take: 50 })
          if (entryResumeGenRef.current !== gen || !allowEntryResumeRef.current) {
            return
          }
          job = pickLatestInFlight(page.items ?? [])
        }

        if (!job || entryResumeGenRef.current !== gen || !allowEntryResumeRef.current) {
          return
        }
  ...
  const handleJobSelect = React.useCallback(
    (jobId: string) => {
      ...
      // Historical row — clear live overlay so Detail uses API history.
  ...
  return (
    <ItemForm
      ...
      onStartNewReport={() => {
        ...
        preferCriteriaRef.current = true
        setComposing(true)
        // Drop hist / live selection so Criteria does not look tied to a past run.
        publishFocused(null)
      }}
