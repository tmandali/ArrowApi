...
describe("resolveGridFastRoute — durum niyeti deterministik bool filtre", () => {
  const statusScreen = (columnTypes: Record<string, string>): ScreenContext =>
    ({
      screenId: "report-grid-status",
      screenTitle: "Stok Bakiyesi Tablosu",
      workspaceId: "stock",
      activeDataSummary: {
        isViewingResults: true,
  ...
  it("'pasif olanlar' → tek bool kolona query=false (model indirilmez)", () => {
    const r = resolveGridFastRoute("pasif olanlar", statusScreen(BOOL_ONE), true)
    expect(r.matched).toBe(true)
    expect(r.toolName).toBe("filter_active_grid")
  ...
  it("'aktif olanlar' → query=true", () => {
    const r = resolveGridFastRoute("aktif olanlar", statusScreen(BOOL_ONE), true)
    expect(r.matched).toBe(true)
    expect(r.args.query).toBe("true")
  })

  it("birden fazla bool kolon → belirsiz, kısayol eşleşmez (modele bırakılır)", () => {
    const r = resolveGridFastRoute("pasif olanlar", statusScreen(BOOL_TWO), true)
    const shortcut =
      r.matched && r.toolName === "filter_active_grid" &&
      r.args.column === "IsActive" && r.args.query === "false"
