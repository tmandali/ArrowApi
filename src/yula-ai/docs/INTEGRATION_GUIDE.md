...
Projenizin `src/` klasörü altına simülasyon test dosyasını ekleyin. Hazır ve tam donanımlı şablon dosyasını [`agent-simulation.template.ts`](./templates/agent-simulation.template.ts) dosyasından doğrudan kopyalayabilirsiniz.

Şablonun temel yapısı şu şekildedir:
...
describe('🤖 Headless UI-Agent Uçtan Uca Simülasyonu', () => {
  ...
  // Test bileşenini ve Zod sözleşmesini kaydet
  uiRegistry.register({
    id: 'filter_form',
    actions: {
      SET_FIELDS: {
        schema: z.object({ storeId: z.string().min(1), dateRange: z.string().regex(/^\d{4}-\d{2}$/) }),
  ...
  it('Aşama 1: Zod Preflight ve Kendi Kendini Onarma', async () => {
