...
import { modelCatalog } from '../knowledge/model-catalog';
import { piEventStream } from '../telemetry/pi-event-stream';
import { formatRelativeAge } from '../ui-bridge/event-bus';
...
## Critical Context
- [Preserve existing, add new context needed]`;
 ...
 * Pi Standart 5 Bölümlü Markdown Şablonuna uygun üretir.
 */
export function generateLocalSummary(serializedText: string): string {
  ...
  const sections: string[] = [
    ...
    toolsExecuted.length > 0
      ? toolsExecuted.map((t) => `- [x] ${t}`).join('\n')
      : '- [x] Initial conversational exchange',
    '',
    '### In Progress',
    ...
    '',
    '## Key Decisions',
    '- **Compacted Context**: Conversation history safely archived into structured summary.',
    '',
    '## Critical Context',
    '- (none)',
  ];
