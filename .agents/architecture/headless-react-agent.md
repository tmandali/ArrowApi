...
20. **Component Event Contracts (`events`) & Topic-Segmented Upstream Telemetry (`TelemetryTopic`, `TELEMETRY_TOPICS` & `AppTelemetryEvent`)**: Declares telemetry and state events emitted by components over `uiEventBus` for observability, reflection tools (`inspect_ui_state`), and bounded ring-buffer memory. Type safety and starvation immunity are enforced across:
    ...
    - *Topic-Balanced Compaction (`getTopicBalancedEvents` & `distinctByType`)*: Retains the latest event per type per topic for balanced interaction snapshots in system prompts and compaction pipelines.
    - *Targeted Agent Inspection (`inspect_ui_state`)*: Enables LLMs to perform targeted queries by `topic`, `source`, or `event_type` (similar to grep/tail filtering) rather than blindly dumping unrelated system events.
