...
- **Headless React UI-Agent Architecture & Pi Reliability Layer (@my-agent)**:
  - **Action Dispatch & Preflight**: Replaces legacy manual tool loops with `@my-agent/react` (`dispatch_component_action`, `inspect_ui_state`, `ask_user_choice`, `remember_fact`, `time_travel`).
