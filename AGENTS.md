...
- **Çok Dilli LLM & Context Poisoning Önleme**: Tool çıktılarındaki tüm sistem geri bildirimleri (`message`, `hint`, `error`, `directive`, `note`) standart İngilizce tutulur. Asla tek bir dili dayatan yönergeler verilmez; istemci tarafında yapay dil regex kapıları konulmaz, yanıt dilini kullanıcının tercihi ve LLM semantiği belirler.
- **Yula AI Panel (Side Dock)**: Yula AI paneli ekran boyutundan bağımsız daima yan dock/çekmece formunu korur; küçük ekranlarda zorunlu tam ekrana geçmez.
